...
*************************************************************************/";
    ...
    void CreateCS(string[] reflectFileName,string JsonFileName)
    {
        ...
        {
            sw.WriteLine($"{tabKey}public {argumentType[i].ToLower()} {argumentName[i]};");
        }
        ...
        //此处根据需求改动i = 4 或者i = 2
        sw.WriteLine($"{tabKey}{tabKey}for (int i = 4; i < fileStr.Length; i++)" + "{");
        ...
        sw.WriteLine($"{tabKey}public bool TryDeleteDataByIndex(int index)" + "{");
        sw.WriteLine($"{tabKey}{tabKey}if({JsonFileName}_list[index]!=null)  "+"{");
        sw.WriteLine($"{tabKey}{tabKey}{JsonFileName}_list.RemoveAt(index);");
        sw.WriteLine($"{tabKey}return true;" + "}");
        sw.WriteLine($"{tabKey}return false;" + "}");

        sw.WriteLine($"{tabKey}public void TryChangeDataByIndex(int index,{JsonFileName} data)" + "{");
        sw.WriteLine($"{tabKey}{tabKey}if({JsonFileName}_list[index]!=null)  " + "{");
        sw.WriteLine($"{tabKey}{tabKey}{JsonFileName}_list[index]=data;");
        sw.WriteLine($"{tabKey}"+ "}");
        sw.WriteLine($"{tabKey}" + "}");
